...
wget https://udomain.dl.sourceforge.net/project/pcre/pcre/8.38/pcre-8.38.tar.gz --no-check-certificate
...
# begin
rm -rf /opt/nginx-1.7.8/conf/nginx.conf
wget -P /opt/nginx-1.7.8/conf https://raw.githubusercontent.com/Yang11291999/google_image/main/nginx.conf # 通过下载模板文件，然后修改ip本机
ServerIp=$(curl http://metadata.tencentyun.com/meta-data/public-ipv4)                                     #获取本机公网ip
sed -i 's/ServerIp/'$ServerIp'/g' /opt/nginx-1.7.8/conf/nginx.conf
# end
